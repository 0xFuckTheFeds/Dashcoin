...
export default function ComparePage() {
  ...
  const [comparisonData, setComparisonData] = useState<{
    token1: ComparisonTokenData | null;
    token2: ComparisonTokenData | null;
  }>({
    token1: null,
    token2: null,
  });
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        token1SuggestionsRef.current &&
        !token1SuggestionsRef.current.contains(event.target as Node)
      ) {
        setShowToken1Suggestions(false);
      }
      if (
        token2SuggestionsRef.current &&
        !token2SuggestionsRef.current.contains(event.target as Node)
      ) {
  ...
  const calculateMarketCapGrowthPerDay = (
    marketCap: number,
    launchDate: string,
  ): number => {
  ...
  const convertTokenData = (token: TokenData): ComparisonTokenData => {
    const marketcapgrowthperday = calculateMarketCapGrowthPerDay(
      token.marketCap,
      token.first_trade_time,
    );
    return {
      address: token.token,
      name: token.name,
  ...
  const handleCompare = () => {
    ...
    setError(null);
    try {
      let token1 = allTokens.find(
        (t) => t.token.toLowerCase() === token1Name.toLowerCase(),
      );
      let token2 = allTokens.find(
        (t) => t.token.toLowerCase() === token2Name.toLowerCase(),
      );

      if (!token1) {
        token1 = allTokens.find(
          (t) =>
      ...
      if (!token1 || !token2) {
        setError(
          "One or both tokens not found. Please check name, symbol, or address and try again.",
        );
        setIsLoading(false);
        return;
      }
  ...
  if (comparisonData.token1 && comparisonData.token2) {
    if (
      comparisonData.token1.marketcapgrowthperday >
      comparisonData.token2.marketcapgrowthperday
    ) {
    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="mb-8 text-center">
          <h1 className="dashcoin-title text-4xl md:text-5xl text-dashYellow mb-4">
            TOKEN COMPARISON
          </h1>
          <p className="text-xl max-w-3xl mx-auto">
            Compare any two tokens to analyze market cap, holders, and other
            metrics
          </p>
        </div>
        ...
        <DashcoinCard className="mb-8">
          <DashcoinCardHeader>
            <DashcoinCardTitle className="text-center">
              Enter Token Names to Compare
            </DashcoinCardTitle>
          </DashcoinCardHeader>
          <DashcoinCardContent>
            <form
            ...
            >
              <div className="flex flex-col md:flex-row gap-4 items-start md:items-end">
                <div className="flex-1 relative" ref={token1SuggestionsRef}>
                  <label
                    htmlFor="token1"
                    className="block mb-2 text-sm font-medium text-center w-full"
                  >
                    Token 1 Name
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      handleTokenSearch(
                        e.target.value,
                        setToken1Name,
                        setToken1Suggestions,
                        setShowToken1Suggestions,
                      )
                    }
                    onFocus={() =>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleTokenSearch(
                        e.target.value,
                        setToken2Name,
                        setToken2Suggestions,
                        setShowToken2Suggestions,
                      )
                    }
                    onFocus={() =>
                ...
                <Button
                  ...
                  className="bg-dashYellow text-dashBlack hover:bg-yellow-500 hover:text-white py-6 px-8 w-full md:w-auto"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
              ...
              <p className="mt-2 text-sm opacity-70 flex items-center gap-2 justify-center">
                <InfoIcon className="h-4 w-4" />
                Enter the token names, symbols, or addresses to compare metrics
              </p>
            </form>
          </DashcoinCardContent>
        </DashcoinCard>
          ...
          <Fragment>
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 p-4 bg-dashGreen-dark rounded-lg border border-dashGreen-light">
              <div className="flex-1 text-center p-4">
                <h3 className="text-xl text-dashYellow">
                  {comparisonData.token1.name} ({comparisonData.token1.symbol})
                </h3>
                <div className="opacity-70 text-sm mt-2">
                  {comparisonData.token1.address.substring(0, 6)}...
                  {comparisonData.token1.address.substring(
                    comparisonData.token1.address.length - 4,
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 text-center p-4">
                <h3 className="text-xl text-dashYellow">
                  {comparisonData.token2.name} ({comparisonData.token2.symbol})
                </h3>
                <div className="opacity-70 text-sm mt-2">
                  {comparisonData.token2.address.substring(0, 6)}...
                  {comparisonData.token2.address.substring(
                    comparisonData.token2.address.length - 4,
                  )}
                </div>
              </div>
            </div>
                    ...
                    <DashcoinCardContent>
                      <div className="h-80 w-1/2 mx-auto">
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            <Legend />
                            <Bar
                              dataKey={comparisonData.token1.symbol}
                              fill={token1MarketCapColor}
                            />
                            <Bar
                              dataKey={comparisonData.token2.symbol}
                              fill={token2MarketCapColor}
                            />
                          </RechartsBarChart>
                        </ResponsiveContainer>
                      </div>
                      <div className="mt-4 grid grid-cols-2 gap-4">
                        <div className="p-3 rounded-md bg-dashGreen-dark">
                          <p className="text-sm opacity-70">{`${comparisonData.token1.name} Market Cap`}</p>
                          <p className="text-xl text-dashYellow">
                            ${comparisonData.token1.marketCap.toLocaleString()}
                          </p>
                        </div>
                        <div className="p-3 rounded-md bg-dashGreen-dark">
                          <p className="text-sm opacity-70">{`${comparisonData.token2.name} Market Cap`}</p>
                          <p className="text-xl text-dashYellow">
                ...
                <div className="flex flex-col gap-4">
                  <DashcoinCard>
                    ...
                    <DashcoinCardContent>
                      <div className="h-80 w-1/2 mx-auto">
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            <Bar
                              dataKey={comparisonData.token1.symbol}
                              fill={token1HoldersColor}
                            />
                            <Bar
                              dataKey={comparisonData.token2.symbol}
                              fill={token2HoldersColor}
                            />
                          </RechartsBarChart>
                        </ResponsiveContainer>
                      </div>
                      <div className="mt-4 grid grid-cols-2 gap-4">
                        <div className="p-3 rounded-md bg-dashGreen-dark">
                          <p className="text-sm opacity-70">{`${comparisonData.token1.name} Holders`}</p>
                          <p className="text-xl text-[#3B4FE4]">
                            {comparisonData.token1.holders.toLocaleString()}
                          </p>
                        </div>
                        <div className="p-3 rounded-md bg-dashGreen-dark">
                          <p className="text-sm opacity-70">{`${comparisonData.token2.name} Holders`}</p>
                          <p className="text-xl text-[#3B4FE4]">
                        ...
                        <tbody>
                          <tr className="border-b border-dashGreen-light">
                            ...
                            {(() => {
                              ...
                              const colorClass = getDifferenceColorClass(
                                v1,
                                v2,
                                multipleStr,
                              );
                              return (
                                <td
                                  className={`text-right py-3 px-4 ${colorClass}`}
                                >
                                  {multipleStr}
                                </td>
                              );
                          ...
                          <tr className="border-b border-dashGreen-light">
                            ...
                            <td className="text-right py-3 px-4">
                              {new Date(
                                comparisonData.token1.launchDate,
                              ).toLocaleDateString()}
                            </td>
                            <td className="text-right py-3 px-4">
                              {new Date(
                                comparisonData.token2.launchDate,
                              ).toLocaleDateString()}
                            </td>
                          ...
                          <tr className="border-t border-b border-dashGreen-light">
                            ...
                            <td className="text-right py-3 px-4">
                              $
                              {(
                                comparisonData.token1.marketCap /
                                comparisonData.token1.holders
                              ).toLocaleString()}
                            </td>
                            <td className="text-right py-3 px-4">
                              $
                              {(
                                comparisonData.token2.marketCap /
                                comparisonData.token2.holders
                              ).toLocaleString()}
                            </td>
                            {(() => {
                              const v1 =
