...
export function FounderMetadataGrid({ token1, token2 }: Props) {
  ...
  const rows = [
    { key: "Research Score", display: "Research Score", val1: token1.score, val2: token2.score, score: true },
    ...canonicalChecklist.map(trait => ({ key: trait.label, display: trait.display, val1: token1[trait.label], val2: token2[trait.label], score: false })),
  ];
  ...
  return (
    <div className="divide-y divide-dashGreen-light">
      ...
      {rows.map(({ key, display, val1, val2, score }) => {
        const s1 = score ? (typeof val1 === 'number' ? val1 : 0) : valueToScore(val1, (gradeMaps as any)[key] || gradeMaps.default);
        const s2 = score ? (typeof val2 === 'number' ? val2 : 0) : valueToScore(val2, (gradeMaps as any)[key] || gradeMaps.default);
        const best1 = s1 > s2;
        const best2 = s2 > s1;
        const classBest1 = best1 ? "border border-dashYellow" : "";
        const classBest2 = best2 ? "border border-dashYellow" : "";
        const col1 = score ? scoreColor(val1 as number | null) : traitColor(val1, key);
        const col2 = score ? scoreColor(val2 as number | null) : traitColor(val2, key);
        return (
          <div key={key} className="flex flex-col md:grid md:grid-cols-3 py-2 gap-2 group">
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="px-2 font-medium cursor-help text-gray-50">{display}</div>
                </TooltipTrigger>
